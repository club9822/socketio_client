...
class Page extends React.PureComponent {
  ...
  leaveRoom = () => {
    if (this.checkSocketRef()) {
      ...
      socketRef.current?.emit(
        "leaveRoom",
        { userId: socketRef.current?.id + "", roomId: this.roomId },
        (e, result) => {
          console.log("leaveRoom=>>>>>>", e, result);
          return;
          this.setState({
            usersJoinedRoom: this.state.usersJoinedRoom.filter(
      ...
      if (socketId) {
        ...
        setTimeout(() => {
          if (this.state.usersJoinedRoom.hasOwnProperty(socketId)) {
            const usersJoinedRoom = Object.assign(
              {},
              this.state.usersJoinedRoom
            );
      ...
      if (socketId) {
        ...
        setTimeout(() => {
          if (this.state.usersLeavedRoom.hasOwnProperty(socketId)) {
            const usersLeavedRoom = Object.assign(
              {},
              this.state.usersLeavedRoom
            );
      ...
      .then((res) => {
        this.setState({
          messages: res.data.map((msg) => ({
            ...msg,
            position: msg?.UserId === this.userId ? "right" : "left",
          })),
        });
      })
      .catch((e) => {})
  ...
  componentDidMount() {
    ...
    this.listeners();
    this.getALlSocketsInRoom();
    this.props.socketRef.current?.emit("joinRoom", {
      roomId: this.roomId,
      username: this.username,
    });
